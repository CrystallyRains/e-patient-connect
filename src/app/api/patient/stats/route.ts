...
export async function GET(request: NextRequest) {
  try {
    ...
    // Determine which patient's stats to retrieve
    ...
    const statsResult = await EncounterService.getEncounterStats(targetPatientId)
    ...
    return NextResponse.json({
      ...
      stats: statsResult.encounter
    })

  } catch (error) {
    console.error('Get patient stats API error:', error)
    return NextResponse.json(
